...
class WindowsCollector(DataCollector):
    """Windows-specific system data collector implementation."""
    ...
    def __init__(
        ...
        """Initialize Windows data collector."""
        ...
    ...
    def _count_conpty(self) -> int:
        """Count conhost.exe instances and estimate Windows Terminal internal pseudo-consoles."""
        ...

    def _get_handle_count(self) -> int:
        """Query aggregate system handle count with psutil or Win32 GetProcessHandleCount fallback."""
        ...
...
```
1. Initialize WindowsCollector with poll_interval (default 2.0s) and thresholds.
2. On start():
   a. Create stop_event (threading.Event).
   b. Launch background thread running _run_loop().
3. In _run_loop():
   a. WHILE not stop_event.is_set():
      i. Start timestamp timer.
      ii. Call poll_metrics():
          - Count conhost.exe processes and WT pseudo-consoles via _count_conpty().
          - Fetch len(psutil.pids()) or process_iter for process_count.
          - Fetch psutil.virtual_memory().percent for memory_percent.
          - Sum handle count via psutil or ctypes Win32 API in _get_handle_count().
          - Check Python cmdlines for 'unleashed-c-*.py' via _count_unleashed_sessions().
          - Calculate composite_value and driver via calculate_composite_metric().
          - Create SystemSnapshot instance.
      iii. Put snapshot into snapshot_queue (non-blocking / drop oldest if queue full).
      iv. Sleep for remaining duration of poll_interval (checking stop_event).
4. On stop():
   a. Set stop_event.
   b. Join background thread with timeout.
```

### 2.6 Technical Approach

* **Module:** `src/boostgauge/collector.py` & `src/boostgauge/collectors/windows.py`
* **Pattern:** Strategy / Abstract Factory pattern for platform collectors (`DataCollector` base class with `WindowsCollector` subclass created via `create_collector()`).
* **Key Decisions:**
  - Use `psutil` as primary provider for process counts, memory %, and process handles.
  - Implement ConPTY estimation by counting `conhost.exe` and checking process parentage for `OpenConsole.exe` / `WindowsTerminal.exe`.
  - Handle permission errors (`psutil.AccessDenied`) gracefully during handle enumeration and command-line inspection by caching last known good metrics or skipping unprivileged processes.
  - Use a non-blocking background thread pushing to `queue.Queue(maxsize=100)` to ensure zero GUI stuttering.
...
| 100 | Single metric exception isolation (REQ-6) | Auto | Mock `_count_conpty()` raising `RuntimeError` | Snapshot created with zero/cached conpty_count | Polling thread remains alive |
...
poetry run pytest --cov=src/boostgauge/collector --cov=src/boostgauge/collectors/windows --cov-report=term-missing tests/
```
...
|------|--------|------------|------------|
| Process iteration overhead on systems with thousands of processes | Med | Low | Filter process iteration by name in `WindowsCollector._count_unleashed_sessions()` before invoking `cmdline()` |
| Win32 handle query permission denial | Low | Med | Catch `psutil.AccessDenied` in `WindowsCollector._get_handle_count()` and fallback to accessible process handles |
| Background thread runaway on queue block | High | Low | Use `put_nowait()` with queue exception handling in `DataCollector._run_loop()` and bounding queue maxsize |
...
Mechanical validation automatically checks:
- Every file mentioned in this section appears in Section 2.1 (`src/boostgauge/__init__.py`, `src/boostgauge/collector.py`, `src/boostgauge/collectors/__init__.py`, `src/boostgauge/collectors/windows.py`, `tests/unit/test_collector.py`, `tests/unit/test_windows_collector.py`, `tests/contract/test_collector_contract.py`)
- Every risk mitigation in Section 11 has a corresponding function signature in Section 2.4 (`WindowsCollector._count_unleashed_sessions()`, `WindowsCollector._get_handle_count()`, `DataCollector._run_loop()`)
...
|----|---------|--------------|
| G1.1 | "Initial LLD draft creation for Windows Data Collector (#4)." | YES - Created comprehensive LLD strictly matching specifications and test strategies. |
...
| Gemini #1 | 2026-08-01 | APPROVED | Initial LLD specification approved |
