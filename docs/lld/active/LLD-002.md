...
class TelltaleStyleSpec:
    color: tuple[int, int, int, int]        # RGBA color tuple with opacity
   ...
   c. DRAW telltale needles for non-None peak values:
      - FOR EACH (key, peak) IN telltales:
        - IF peak IS NOT None THEN
          ...
          - Draw needle line from pivot to outer dial using TelltaleStyleSpec (colors: cyan, orange, magenta, red with alpha).
...
- Must adhere strictly to Option C testing strategy in `docs/design/0001-test-strategy.md` (no `tkinter.Tk()` initialization during testing).
...
3. Map each telltale's `current_peak()` value to an angular position on the gauge dial face based on gauge geometry.
...
|------|--------|------------|------------|
| Anti-aliasing noise causing visual test failures across OS platforms | Med | Low | RMS pixel-diff tolerance band (RMS ≤ 1.0/255) per test strategy `docs/design/0001-test-strategy.md` |
