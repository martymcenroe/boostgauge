...
## 1. Context & Goal
* **Issue:** #2
* **Objective:** Render four peak-hold (telltale) needles (1m, 10m, 1h, all-time) on top of the PIL gauge surface behind the main needle, consuming `Telltale` instances from #41.
...
|------|-------------|-------------|
| `tests/visual/baselines` | Add (Directory) | Baseline directory for visual regression test image snapshots. |
...
| `src/boostgauge/gauge.py` | Add | Tachometer gauge renderer handling dial face composition, main needle drawing, and telltale layer composition. |
| `src/boostgauge/skins/stingray.py` | Add | Stingray skin renderer drawing dial, tick marks, main needle, and four distinct telltale needles (1m cyan translucent, 10m orange translucent, 1h magenta dashed, all-time red solid). |
...
| `tests/visual/test_gauge.py` | Add | Render-pixel visual regression tests for active telltales, post-reset missing needle suppression, and main needle z-order layering. |
...
```python
from typing import Dict, Literal, Optional, TypedDict, Tuple
...
* **Pattern:** Facade / Composite Layered Renderer
* **Key Decisions:**
  - `TelltaleManager` encapsulates `Telltale` instances from #41 and provides pure data structures to `render()` to adhere strictly to Option C (off-screen PIL rendering without GUI/Tkinter coupling).
...
3. Render four telltale needles on the PIL gauge surface based on `Telltale.current_peak()` values using visually distinct styles: thin translucent cyan/light blue for 1m (60s), orange translucent for 10m (600s), magenta/purple dashed for 1h (3600s), and solid red for all-time (`None`).
...
**Coverage Target:** ≥95% coverage on touched files (`src/boostgauge/telltale.py`, `src/boostgauge/gauge.py`, `src/boostgauge/skins/stingray.py`).
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Instantiate `TelltaleManager` with 4 window configurations (REQ-1) | Auto | Constructor call | Manager holding 4 `Telltale` instances with windows (60, 600, 3600, None) | All 4 window instances created and configured correctly |
| 020 | Forward live metric sample updates to all windows (REQ-2) | Auto | `update(t=100.0, val=75.0)` | All 4 telltales receive sample and return 75.0 peak | `get_peaks()` returns `{'m1': 75.0, 'm10': 75.0, 'h1': 75.0, 'all': 75.0}` |
| 030 | Render distinct telltale needles on PIL surface (REQ-3) | Auto | Peaks: m1=40, m10=60, h1=80, all=95 | Rendered PIL `Image` with cyan, orange, magenta, and red needles | Image pixel-diff matches baseline snapshot |
| 040 | Verify main needle overlay z-order on top of telltales (REQ-4) | Auto | Main value 60.0 overlapping telltale at 60.0 | Main needle pixel color visible at overlap point | Main needle drawn on top of telltale needle layer |
| 050 | Post-reset missing needle rendering suppression (REQ-5) | Auto | Reset `m1` window, peak returns `None` | PIL `Image` rendered without 1m needle | Cyan needle completely absent from render |
| 060 | Context menu reset execution for individual and all windows (REQ-6) | Auto | Call `reset('m1')` and `reset_all()` | Target peak reset to `None` | `current_peak()` returns `None` for reset targets |
| 070 | 1m sliding window peak eviction after 60 seconds (REQ-7) | Auto | Sample 90 at t=0, sample 50 at t=65 | `m1` peak returns 50.0 after 65 seconds | 1m peak drops to highest remaining in-window sample |
| 080 | Indefinite retention of all-time peak (REQ-8) | Auto | Sample 95 at t=0, sample 40 at t=4000 | `all` peak returns 95.0 after 4000 seconds | All-time peak remains 95.0 |
...
- [ ] Coverage meets target on touched files
