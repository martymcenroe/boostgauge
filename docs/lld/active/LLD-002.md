...
* **Related Issues:** #1, #4, #7, #26, #27, #28, #29, #41
...
| `src/boostgauge/gauge.py` | Modify | Core gauge entry point function `render()` dispatching state to skin renderers. |
...
| `tests/contract/test_telltale_contract.py` | Add | Contract tests validating `TelltaleManager` public API and signature contracts for `render()`. |
