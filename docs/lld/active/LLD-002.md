...
3. Render four telltale needles on the PIL gauge surface based on `Telltale.current_peak()` values using distinct visual styles: thin translucent cyan for 1m (60s), thin translucent orange for 10m (600s), thin dashed magenta for 1h (3600s), and thin solid red for all-time (`None`).
4. Render all telltale needles behind the main dynamic gauge needle in z-order on the PIL Image surface.
...
**Coverage Target:** ≥95% for all new code per `docs/design/0001-test-strategy.md`.
...
- `src/boostgauge/telltale.py`: Defined in 2.1, 2.4, 2.6
- `src/boostgauge/gauge.py`: Defined in 2.1, 2.4
- `src/boostgauge/skins/stingray.py`: Defined in 2.1, 2.4, 2.6
