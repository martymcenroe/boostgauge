...
### Open Questions
- None. Requirements and visual specs aligned with Issue #2, Issue #41, and `docs/design/0001-test-strategy.md`.
...
* **Key Decisions:**
  1. **Option C Compliance:** The renderer operates strictly on `PIL.Image` objects off-screen per `docs/design/0001-test-strategy.md`. `tkinter.Tk()` or Canvas widgets are never instantiated or referenced inside the render path.
...
1. Instantiate four `Telltale` instances with sliding windows of 60s (1m), 600s (10m), 3600s (1h), and None (all-time).
2. Pipe live metric stream samples into all four `Telltale` instances via `Telltale.update(timestamp, value)`.
3. Render four distinct telltale needles (1m: Cyan/light blue thin translucent, 10m: Orange thin translucent, 1h: Magenta/purple thin dashed, All-time: Red thin solid) on top of the gauge surface.
4. Calculate angular needle position deterministically from `Telltale.current_peak()` mapped across the gauge scale geometry.
5. Skip rendering any telltale needle whose `Telltale.current_peak()` returns `None` (post-reset or prior to first sample).
6. Render telltale needles behind the main needle (z-order) on an off-screen `PIL.Image` surface per Option C of `docs/design/0001-test-strategy.md` without instantiating `tkinter.Tk()`.
7. Handle right-click context menu reset actions to call `reset()` on individual telltales ("Reset 1m", "Reset 10m", "Reset 1h", "Reset All-time") or all telltales ("Reset All").
8. Ensure sliding window peaks (1m, 10m, 1h) drop back to highest in-window value as samples age out, while all-time peak never drops without explicit reset.
9. Render a small color-coded legend on the gauge surface identifying the four telltale needle colors and windows.
...
| Off-screen baseline images | `tests/visual/baselines/*.png` | Visual regression baselines for render testing (requires `--generate-baselines` flag) |
...
Testing strictly adheres to `docs/design/0001-test-strategy.md` utilizing Option C (off-screen `PIL.Image` rendering with zero `tkinter.Tk()` instantiation). Visual regression baselines under `tests/visual/baselines/` require an explicit `--generate-baselines` flag — no implicit auto-acceptance.
