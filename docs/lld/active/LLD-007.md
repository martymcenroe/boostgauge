...
2. Determine target config file path:
   IF CLI --config PATH is specified THEN
       target_path = Path(CLI --config).resolve()
   ELSE
...
* **Key Decisions:**
  ...
  - Implement atomic file replacement (`.tmp` write in same target directory followed by `os.replace`) to protect against corrupted `config.json` files on abrupt application termination across filesystem boundaries.
...
2. Custom config file path can be specified via `--config PATH` CLI argument, overriding default configuration file location.
3. CLI argument `--reset-config` resets configuration file to default values upon execution.
4. CLI options (`--theme`, `--size`, `--poll`, `--opacity`, `--no-topmost`) override corresponding settings loaded from configuration file.
5. Application updates and persists window position (`position.x`, `position.y`) and size (`size`) to active configuration file upon window exit/shutdown.
6. Restored window position and size geometry from configuration file are applied when application launches.
7. Threshold configurations (`conpty`, `memory_percent`, `process_count`, `handle_count`) update dynamically in memory without requiring application restart.
...
| Concern | Mitigation | Status |
|---------|------------|--------|
| Corrupted `config.json` on power failure | Atomic file write using temporary file in same directory and `os.replace` in `save_config_atomic()` | Addressed |
