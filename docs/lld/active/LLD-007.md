...
|------|-------------|-------------|
| `src/boostgauge` | Add (Directory) | Package source directory for boostgauge modules |
| `src/boostgauge/__init__.py` | Add | Package initialization file defining export symbols and version |
| `src/boostgauge/config.py` | Add | Core configuration manager supporting defaults, loading, saving, validation, and CLI overrides |
...
- All "Add" files must have existing parent directories (`src/` and `tests/unit/` exist)
...
# pyproject.toml additions (using standard library argparse and json; no new third-party packages required)
[tool.poetry.scripts]
...
# Function signatures in src/boostgauge/config.py
...
1. Application Startup (main):
   ...
   f. Validate final combined configuration via validate_config(). On failure, output error to stderr and exit with non-zero code.

2. Runtime Threshold Modification:
   a. When threshold settings change during runtime (e.g. via IPC, hot-reload, or API):
   ...
   d. Trigger renderer and telltale state re-evaluation without restarting app.
...
* **Pattern:** Data Transfer Object (TypedDict) with atomic File I/O manager and Layered Override Pipeline (Defaults -> File -> CLI).
* **Key Decisions:**
  - Standard Python `json` and `argparse` libraries are used to keep `boostgauge` lightweight without extra external dependencies.
  - Configuration path resolution selects `%APPDATA%/boostgauge/config.json` on Windows (using `os.environ.get("APPDATA")`) and `~/.boostgauge/config.json` on Linux/macOS.
  - File saving uses atomic write semantics (writing to `.tmp` file and renaming) to prevent corrupted JSON if app exits or crashes mid-write.

### 2.7 Architecture Decisions

| Decision | Options Considered | Choice | Rationale |
|----------|-------------------|--------|-----------|
| Config Format | JSON vs TOML vs YAML | JSON | Requirements explicitly define JSON schema; built-in standard library support without adding dependencies |
| Schema Validation | Pydantic vs dataclasses vs TypedDict + explicit validator | TypedDict + explicit validator | Zero additional dependency overhead; crisp error messages tailored to CLI/GUI feedback |
| CLI Parsing | Click / Typer vs argparse | argparse | Built into standard library, zero runtime footprint, natively handles overrides |

**Architectural Constraints:**
- Must conform to `docs/design/0001-test-strategy.md` for test coverage and GUI isolation.
- Zero new runtime external dependencies beyond PyPI baseline (`psutil`, `pillow`, `pystray`).
...
2. CLI arguments (`--theme`, `--size`, `--poll`, `--opacity`, `--no-topmost`) override corresponding configuration values loaded from disk or defaults.
...
6. Dynamic threshold updates and configuration changes re-validate and take effect immediately in memory without requiring an application restart.
...
|--------|------|------|----------|
| Pydantic for Config Validation | Built-in validation, clean syntax | Adds large external dependency to lightweight app | Rejected |
| Single YAML configuration file | Human-friendly comments | Requires PyYAML dependency | Rejected |
| Standard JSON file with custom validator | Zero external dependencies, exact match to issue spec | Manual validator code required | **Selected** |

**Rationale:** Using standard library `json` and `argparse` with custom `validate_config()` maintains minimal binary size and zero extra dependencies for PyInstaller releases.
...
|---------|--------|-------|
| `valid_config_json` | Hardcoded in `tests/unit/test_config.py` | Full valid config dictionary matching issue spec |
| `invalid_config_json` | Hardcoded in `tests/unit/test_config.py` | Malformed JSON, out-of-bounds numbers, unknown themes |
| `tmp_config_dir` | pytest `tmp_path` fixture | Isolated temporary directory for file I/O tests |

### 5.4 Deployment Pipeline

No external deployment pipeline needed. Configuration defaults ship with the `boostgauge` package wheel/executable.
...
|---------|------------|--------|
| Directory Traversal via `--config` | Sanitize and resolve `Path(config_path).resolve()` | Addressed |
| Malicious JSON Injection | Strict schema validation with type and boundary checks; reject extra/unexpected types | Addressed |

### 7.2 Safety

| Concern | Mitigation | Status |
|---------|------------|--------|
| Partial file write on crash/exit | Atomic file write pattern: write to `.tmp` file in destination folder then `os.replace` | Addressed |
| Corrupted config crashes app | Catch JSONDecodeError / ConfigError, log clear message, fall back to defaults or fail gracefully | Addressed |

**Fail Mode:** Fail Closed with descriptive error to stderr when CLI arguments or config content are invalid.

**Recovery Strategy:** Re-run with `--reset-config` to overwrite invalid disk configuration with valid default configuration.
...
| Third-Party Licenses | No | Only standard library used for config system |
| Terms of Service | No | Standalone client application |
| Data Retention | No | Local configuration file retained until deleted |
...
**Coverage Target:** ≥95% for `src/boostgauge/config.py`.
...
| 020 | Platform-specific path resolution (REQ-1) | Auto | OS environment variables | Correct default `Path` object | Standard path for OS selected |
...
| Missing parent directory for custom config path | Medium | Medium | Auto-create parent directories (`parents=True, exist_ok=True`) prior to saving |
...
Mechanical validation automatically checks:
- Every file mentioned in this section appears in Section 2.1 (`src/boostgauge`, `src/boostgauge/__init__.py`, `src/boostgauge/config.py`, `src/boostgauge/__main__.py`, `src/boostgauge/app.py`, `pyproject.toml`, `tests/unit/test_config.py`)
...
| Gemini #1 | 2026-07-28 | APPROVED | Initial LLD creation for Issue #7 |
