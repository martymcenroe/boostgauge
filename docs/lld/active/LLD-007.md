...
23. A non-threshold key (e.g. `theme`) edited directly in the config file while the app runs leaves the running session unchanged; the edited value takes effect at the next launch.
...
| 051 | Non-threshold key not hot-reloaded (REQ-23) | Auto | Direct edit theme="light" during session | Running session unchanged; next launch uses "light" | `active_config["theme"]` unchanged mid-session; is "light" after relaunch |
