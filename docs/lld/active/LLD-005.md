...
1. Always-on-top window attribute configuration (`root.attributes('-topmost', True)`) surviving window manager focus changes, with toggle option via right-click context menu.
2. Frameless window without OS title bar decorations (`overrideredirect(True)`), draggable by clicking and dragging anywhere on the gauge face, with double-click toggling between compact (150px) and expanded modes.
3. Circular transparent background outside dial face using background color transparency (`root.attributes('-transparentcolor', bg_color)` on Windows), with adjustable hover opacity (e.g. 80% default/unhovered, 100% on mouse hover).
4. System tray integration via `pystray` displaying simplified color indicator dots (green/yellow/red) based on resource composite load score, double-click restore from tray, and right-click context menu (settings, reset telltales, quit).
5. Window geometry persistence (X position, Y position, size) across application restarts saved to configuration file via `config.py`, multi-monitor aware with screen boundary clamping to prevent off-screen placement.
...
| `pystray` system tray event loop blocking Tkinter main loop | High | Low | Run `pystray.Icon` in a dedicated background daemon thread (`threading.Thread(target=icon.run, daemon=True)`). |
...
- [x] Test coverage meets threshold (≥89%)
...
| Gemini Architect #1 | 2026-07-30 | DRAFT | Initial submission |
